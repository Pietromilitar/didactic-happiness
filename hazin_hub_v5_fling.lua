--// HAZIN HUB V5 - 100% LOCALSCRIPT COM GITHUB
--// Key system + painel melhorado + minimizar + fling local
...
openPanel = function()
	...
	label(playerPage, "Fling")

	toggle(playerPage, "Fling Local", "Fling", function(on)
		local _, hum = getChar()
		if hum then
	...
	input(playerPage, "Fling Power", values.FlingPower, function(txt)
		values.FlingPower = tonumber(txt) or 85
	end)

	button(playerPage, "Fling Burst", function()
		local _, hum, root = getChar()
		if root then
			local old = root.AssemblyAngularVelocity
			root.AssemblyAngularVelocity = Vector3.new(0, values.FlingPower * 2.2, 0)
			root.AssemblyLinearVelocity += Vector3.new(0, 35, 0)
			task.delay(0.35, function()
				if root and root.Parent then
					root.AssemblyAngularVelocity = old
				end
			end)
		end
	end)
...
RunService.RenderStepped:Connect(function()
	...
	if states.Fling then
		if not flingAngular or flingAngular.Parent ~= root then
			if flingAngular then
				flingAngular:Destroy()
			end

			flingAngular = Instance.new("BodyAngularVelocity")
			flingAngular.Name = "HazinFlingAngular"
			flingAngular.MaxTorque = Vector3.new(0, math.huge, 0)
			flingAngular.P = 999999
			flingAngular.Parent = root
		end

		flingAngular.AngularVelocity = Vector3.new(0, values.FlingPower, 0)

		-- pequeno impulso para o personagem não "morrer parado"
		if hum.MoveDirection.Magnitude > 0 then
			root.AssemblyLinearVelocity += hum.MoveDirection * 0.75
		end
	else
		if flingAngular then
			flingAngular:Destroy()
